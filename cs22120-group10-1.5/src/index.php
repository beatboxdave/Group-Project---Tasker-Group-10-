		...
		<div id="wrapper" class="contMain">
			<div id="TabbedPanels1" class="TabbedPanels">
				<ul id="panel" class="TabbedPanelsTabGroup">
					...
					<li class="TabbedPanelsTab" tabindex="0"><h2>Allocated</h2></li>
					<li class="TabbedPanelsTab" tabindex="0"><h2>All Tasks</h2></li>
				</ul>
				<div id="contLoad" class="TabbedPanelsContentGroup">
						...
						<?php
							if(isset($_SESSION['name'])){
								...
								<form method="post" onSubmit='return validateMembers()' name="addMember">
									...
									<label>Email: </label><input style="width:20%;" placeholder="Email" class='nmeField' name="memberEmail" type="text" required> </input>
								...
								$sql = "SELECT * FROM team_member";
									...
									while($row = mysqli_fetch_assoc($result)) {
										echo '<tr>';
											...
											echo '<td class="email"><input name="emailEdit" type="text" value="'.$row["email"].'"/></td>';
											echo '<td class="admin">';
						...
						<?php
							if(isset($_SESSION['name'])){
								echo "<h2> Task Status </h2>";
									$sql = "SELECT * FROM task_data WHERE status='Allocated' ORDER BY expEndDate ASC";
									$result = mysqli_query($conn, $sql);
									echo"<table class='task-dat-table'>";
									echo"<tr class='heading'>";
										echo"<td>Title</td>";
					...
					<div id="three" class="TabbedPanelsContent">
						<?php
							if(isset($_SESSION['name'])){
								echo "<h2> Task Status </h2>";
									$sql = "SELECT * FROM task_data WHERE status='Completed' OR status='Abandoned' ORDER BY expEndDate";
									$result = mysqli_query($conn, $sql);
									echo"<table class='task-dat-table'>";
									echo"<tr class='heading'>";
										echo"<td>Title</td>";
